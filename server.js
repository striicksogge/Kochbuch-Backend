...
app.options("*", cors()); // Explizite Vorab-Anfrage-Behandlung, zur Sicherheit (manche Hosting-Umgebungen behandeln automatisches OPTIONS nicht zuverlässig)
// type: () => true zwingt express.json() dazu, JEDEN eingehenden Body als JSON
// zu parsen, unabhängig vom Content-Type-Header. Grund: das Frontend schickt
// bewusst "text/plain" statt "application/json" als Content-Type, um die
// browserseitige CORS-Preflight-Anfrage (OPTIONS) zu vermeiden - auf dieser
// Render-Instanz liefert diese Vorab-Anfrage aus unbekanntem Grund einen 404
// zurück, obwohl der Server selbst korrekt läuft. "text/plain" zählt als
// CORS-"simple request" und braucht deshalb keine Preflight-Prüfung.
